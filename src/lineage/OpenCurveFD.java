...
public class OpenCurveFD {
  ...
  /**
    ...
    */
  public OpenCurveFD(double[] x, double[] y, int n){
      this.x = x;
      this.y = y;
      this.nFD = n;
      this.m = x.length;
  ...
  /**
    ...
    * @param y the y coordinates of the contour
    */  
  public OpenCurveFD(double[] x, double[] y){ 
      this.x = x;
      this.y = y;
      this.nFD = x.length/2;
      this.m = x.length;
  ...
  private void computeEllipticFD(){
    ...
    for (int k = 2; k < nFD; k++){
//      openCurveFd[k] = Math.sqrt((ax[k]*ax[k] + ay[k]*ay[k])/denomA) + 
