...
public class ROF_Denoise {
	 ...
	 */
	public boolean run(ImageProcessor ip) {
		GenericDialog gd = new GenericDialog("ROF Denoise");
		gd.addNumericField("Theta", 25, 2);
		gd.showDialog();
		if (gd.wasCanceled())
			return false;		
		StackConverter stackConverter = new StackConverter(image);
		stackConverter.convertToGray32(); //only convert if we are running the denoise step, chosen by user		
