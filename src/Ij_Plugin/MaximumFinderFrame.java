...
public class MaximumFinderFrame extends JPanel implements DialogListener,AdjustmentListener,ActionListener {
    public MaximumFinderFrame(String title) {
		//super(title);
    ...
    private JLabel     messageArea;                  // reference to the textmessage area for displaying the number of maxima
	...
	List list = new List();
	Hashtable<String, Roi> rois = new Hashtable<String, Roi>(10);
...
//		Label minLabel = new Label("Tolerence", Label.LEFT);
...
//		addLabel("Minimum", null);
		...
		tolerenceSlider = new JScrollBar(Scrollbar.HORIZONTAL, sliderRange/2, 1, 0, sliderRange);
		tolerenceSlider.setPreferredSize(new  Dimension(100, 20));
		tolerenceSlider.addAdjustmentListener(this);
		
		messageArea= new JLabel(String.valueOf(tolerenceSlider.getValue()));
		addRoiButton = new JButton("Add to Roi");
		...
		addComponent(panel, 1, 0, 1, 1, 4, addRoiButton);
		addComponent(panel, 1, 1, 1, 1, 4, messageArea);
		addComponent(panel, 2, 0, 1, 1, 4, previewCheckBox);
		
		panel.setBorder(BorderFactory.createEmptyBorder(10,10,10,10));	
		add(panel);
		
		tolerance = minSliderValue;				
		outputType = POINT_SELECTION;
		messageArea.setText(String.valueOf(tolerance));
    ...
    public int showDialog(ImagePlus imp, String command, PlugInFilterRunner pfr) {
        ...
        messageArea = (JLabel)gd.getMessage();
    ...
    public void run(ImageProcessor ip) {
        ...
        impMax = new ImagePlus(outname, outIp);
        Calibration cal = imp.getCalibration().copy();
        cal.disableDensityCalibration();
        //impMax.setCalibration(cal);             //keep the spatial calibration
        //impMax.show();        
     } //public void run
    ...
    public Polygon getMaxima(ImageProcessor ip, double tolerance, boolean excludeOnEdges) {
		findMaxima(ip, tolerance, ImageProcessor.NO_THRESHOLD,
			MaximumFinderFrame.POINT_SELECTION, excludeOnEdges, false);
		if (points==null)
			return new Polygon();
		else
			return points;
   ...
   /** Get estimated "true" height of a maximum or saddle point of a Euclidian Distance Map.
	...
	public void adjustmentValueChanged(AdjustmentEvent e) {
		...
		if (source==tolerenceSlider)
			minSliderValue = tolerenceSlider.getValue();
		else {
			return;
		}
		tolerance = minSliderValue;				
		...
		messageArea.setText(String.valueOf(tolerance));
		if(previewing == true)
			run(WindowManager.getCurrentImage().getProcessor());
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (source==addRoiButton)
		{
			...
			{				
				...
				{
					PointRoi roi = new PointRoi(points.xpoints[i],points.ypoints[i]);
					roi.setPosition(imp.getStackSize());
					System.out.println("roi position : "+i+ " "+ roi.getPosition());
					roiManager.add(imp, roi, i+1);
				};
				IJ.showMessage("Number of Maxima: "+String.valueOf(points.npoints));
			}
			else {
				IJ.showMessage("output image null!");
			}
		}
	...
	public static void main(String[] ars)
	{
		...
		MaximumFinderFrame maximumFinderFrame = new MaximumFinderFrame("Maxium Frame");
		maximumFinderFrame.setup("", imp);
		maximumFinderFrame.setOpaque(true);	
		JFrame frame = new JFrame("Bacterial analyzer");
		frame.setContentPane(maximumFinderFrame);
		frame.pack();
