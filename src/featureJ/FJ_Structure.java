...
public class FJ_Structure implements PlugIn, WindowListener {
	...
	public ImagePlus[] getEigenImg(ImagePlus imp, String sscale, String iscale) {
		return (new FJStructure()).getEigenImg(imp,sscale,iscale);
	}
...
class FJStructure { // To avoid exceptions when ImageScience is not installed
	...
	ImagePlus[] getEigenImg(final ImagePlus input, final String sscale, final String iscale) {
		try {
			double sscaleval, iscaleval;
			try { sscaleval = Double.parseDouble(sscale); }
			catch (Exception e) { throw new IllegalArgumentException("Invalid smoothing scale value"); }
			...
			for (int i=0; i<size; ++i)
				eigenimages.get(i).aspects(aspects);			
			if (size == 2) {
				return new ImagePlus[]{eigenimages.get(0).imageplus(), eigenimages.get(1).imageplus()};
			} else if (size == 3) {
				return new ImagePlus[]{eigenimages.get(0).imageplus(), eigenimages.get(1).imageplus(),eigenimages.get(2).imageplus()};				
			}
			FJ.close(input);
		} catch (OutOfMemoryError e) {
			FJ.error("Not enough memory for this operation");
